                ...
                <div id="ProductSpecialEvent" class="carousel slide" data-bs-ride="carousel" data-bs-interval="10000">
                    <div class="carousel-indicators">
                        @foreach ($banners as $key => $banner)
                            <button type="button" data-bs-target="#ProductSpecialEvent" data-bs-slide-to="{{ $key }}"
                                    {{ $key === 0 ? 'class=active' : '' }} aria-label="Slide {{ $key + 1 }}"></button>
                        @endforeach
                    </div>
...
@section('script')
    <script>
        $(document).ready(function () {
            let selectedBrands = localStorage.getItem('selectedBrands');
            if (selectedBrands) {
            ...
            $('.brand-card').click(function () {
                ...
                let selectedBrands = $('.brand-card.selected').map(function () {
            ...
            function redirectToPage(selectedBrands) {
                let baseUrl = "{{ route('brands.filter', ['brandSlug' => '']) }}";

                selectedBrands = selectedBrands.filter(Boolean);

                let brandSlug = selectedBrands.join('/');
                window.location.href = baseUrl + '/' + brandSlug;
